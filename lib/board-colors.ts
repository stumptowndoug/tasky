...
export function getBoardColor(
  boardId: string
): (typeof BOARD_COLOR_PALETTE)[number] {
  const colorId =
    DEFAULT_BOARD_COLORS[boardId] || getColorForUnknownBoard(boardId)
  return (
    BOARD_COLOR_PALETTE.find((c) => c.id === colorId) || BOARD_COLOR_PALETTE[0]
  )
}
...
export function getBoardColors(
  boards: Board[]
): Map<string, (typeof BOARD_COLOR_PALETTE)[number]> {
